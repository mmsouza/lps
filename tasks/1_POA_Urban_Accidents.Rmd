...
```{r}
 df %>% select(FX_HORA, FATAIS, CONT_ACID) %>% group_by(FX_HORA) %>% summarise(sum(FATAIS),sum(CONT_ACID)) %>% set_colnames(c("FX_HORA", "FATAIS_COUNT","CONT_ACID")) %>%
  mutate(FATALITIES_RATIO = FATAIS_COUNT/CONT_ACID) %>% ggplot(aes(x=FX_HORA,y=FATALITIES_RATIO)) +geom_col(fill = "#CC0000") +scale_x_continuous(breaks = seq(0, 23, by = 1)) + scale_y_continuous(breaks = seq(0, 30, by = 0.01))
